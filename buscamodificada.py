...
# Instanciar o problema e o algoritmo de busca
no_vaslui = No('Vaslui', 0, None, None)

problema_romenia = Problema(estados_romenia,
                            no_vaslui,
                            lambda no: no.estado == 'Craiova')

busca = BuscaLargura(problema_romenia)

# Adicionar visualização da fronteira passo a passo
class BuscaLarguraModificada(BuscaLargura):
    def passo_busca(self):
        if self.situacao == BUSCA_FALHOU:
            print("Busca falhou")
            return
        ...
        # Visualizar a fronteira atual
        print(f"Fronteira: {[str(n) for n in self.fronteira]}")
...
class BuscaProfundidade:
    ...
    def passo_busca(self):
        ...
        try:
            no = self.fronteira.pop()  # Mudança principal: usar pop() para pegar o último elemento da lista
        except IndexError:
            self.situacao = BUSCA_FALHOU
            return
        ...
        for filho in no.filhos(self.problema):
            if not (filho in self.fronteira) and not (filho.estado in self.visitados):
                self.fronteira.append(filho)  # Adicionar na fronteira
                self.visitados.append(filho.estado)

        return
    
    # Instanciar o problema e o algoritmo de busca em profundidade
no_vaslui = No('Vaslui', 0, None, None)
...
# Executar a busca em profundidade com visualização da fronteira
